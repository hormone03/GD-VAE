...
class DVAE(object):
    ...
    def __init__(self, 
        ...
        lda=False
        self.x = tf.placeholder(tf.float32, [None, vocab_size], name='input') #dataType, shape : None means any no of samples, name
        ...
        with tf.variable_scope('encoder'): 
          self.enc_vec = utils.mlp(self.x, [self.n_hidden], self.non_linearity) #using Multilayer perceptron; the fully connected linear layer
        ...
        with tf.variable_scope('decoder'):
          if self.n_sample ==1:  # single sample
            ...
            dir1=tf.contrib.distributions.Dirichlet(self.prior) #distribution one
            dir2=tf.contrib.distributions.Dirichlet(self.alpha) #distribution two
            self.kld = dir2.log_prob(self.doc_vec)-dir1.log_prob(self.doc_vec) #get the difference btw the distributions
            max_kld_sampled = tf.arg_max(self.kld,0)
            # multiple samples
          else:
            # Me: Calculating Z_tilde, in section 3.4
            # NB: my_random_gamma is a function I implemented, not a library
            # Me: squeeze will remove 1 dim. example: (3, 2, 1) -> (3, 2).
            # Me convert B to tensor of float32
            # Me: shape=(1,) 1 is the number of sample
            ...
            for i in range(self.n_sample):
              ...
              curr_gam = gam[i] #Me: Current Z_tilde
              # reverse engineer the random variables used in the gamma rejection sampler
              # Me: calc_epsilon cal episelon, eq2: make episelon subj of fomular.. h(epselon, alpha) is Z_tilde
              # Me: stop_gradient -> .detach() in pytorch i.e with no gradient
              eps = tf.stop_gradient(calc_epsilon(curr_gam,self.alpha+tf.to_float(self.B)))
              curr_u = u[i]
              # Me: gamma_h_boosted calculate z inside section 3.4 with U_i, z_tilder there now implies EQ2
              ...
              # Me: kld is the diff. between Dir(alpha_enconderOutput) - Dir(alpha_prior)
              # Me: log_prob returns the log of the probability density/mass function evaluated at the given sample value.
              # Me: It is also used to avoid zero because the Dirichlet prior cannot be applied to this distribution because it doesn't allow zero probabilities.
              # Me: Example: b.log_prob(c) is the log of the probability density/mass function evaluated at value (c)
            ...
            # Me: Adds all loss and find average.
        ...
        if self.n_sample ==1:
          #No reverse engineer gaian, we use the eps calculated already at the top
          # this is the sampled gamma for this document, boosted to reduce the variance of the gradient
          ...
          with tf.variable_scope("decoder", reuse=True):
            ...
            for i in range(self.n_sample):
              curr_gam = gam[i]
              #Here, we are calculating eps again because it will be different for each sample
              eps = tf.stop_gradient(calc_epsilon(curr_gam,self.alpha+tf.to_float(self.B)))
              curr_u = u[i]
              self.doc_vec = gamma_h_boosted(eps,curr_u,self.alpha,self.B)
              self.doc_vec = tf.div(self.doc_vec,tf.reshape(tf.reduce_sum(self.doc_vec,1), (-1, 1)))
