...
def compare_images(VariationalAutoEncoder,
        ...
        for i in range(number_original_images):
            ...
            plt.imshow(image.detach().cpu().numpy(), cmap='gray', interpolation='none')
            ...
            for _ in range(number_generated_images):
                ...
                image_reconstructed = VariationalAutoEncoder.reconstruct_img(image).detach().cpu().numpy().reshape(28,28)
...
def t_SNE(test_loader, variational_autoencoder, use_cuda=False):
    ...
    for batch_idx, (example_data, example_targets) in examples:
        if use_cuda:
            ...
            example_targets = example_targets.cuda()
        for idx in range(example_data.shape[0]):

            x = example_data[idx][0]
            z_loc, z_scale = variational_autoencoder.encoder(x)

            values = np.concatenate((z_loc[0].detach().cpu().numpy(), z_scale[0].detach().cpu().numpy()))
            features.append(values)
            label = example_targets[idx].detach().cpu().numpy()
            labels.append(label)
...
def describe_statistic_per_label(test_loader, variational_autoencoder, use_cuda=False):
...
def show_confusion_matrix(test_loader, reconstructor, classifier, use_cuda=False):
    ...
    for batch_idx, (example_data, example_targets) in examples:
        ...
        recon_batch_test, mu, logvar, gauss_z, dir_z = reconstructor(example_data)
        outputs = classifier.predict_proba(recon_batch_test.view(-1, 784).detach().cpu())

        for idx in range(example_data.shape[0]):

            current_prediction = outputs[idx]#.cpu().numpy()
            y_pred.append(current_prediction)

            label = example_targets[idx].cpu().numpy()
            y_test.append(label)
