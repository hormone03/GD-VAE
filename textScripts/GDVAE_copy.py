...
class  DVAE(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope("decoder", reuse=True):
            ...
            self.kld2 = tf.contrib.distributions.Dirichlet(self.alpha).log_prob(
                self.doc_vec) - tf.contrib.distributions.Dirichlet(self.prior_alpha).log_prob(self.doc_vec)
        ...
        if not correction:
            ...
            enc_grads = [g_r + g_c + g_c2 + self.warm_up * g_e for g_r, g_c, g_c2, g_e in
                         zip(g_rep, g_cor, g_cor2, kl_grad)]
